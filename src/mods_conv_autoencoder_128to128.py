...
x = Conv2D(64, (16, 2), activation='relu', padding='same', strides=1)(x)
x = UpSampling2D((2, 1))(x)
x = Conv2D(128, (32, 1), activation='relu', padding='same')(x)
...
with open("../data/decoded_mods_" + datetime.datetime.now().strftime("%m-%d--%H-%M") + ".pkl", 'wb') as f:
    pickle.dump(decoded_mods, f, pickle.HIGHEST_PROTOCOL)
with open("../data/x_test_samples_by_mod_" + datetime.datetime.now().strftime("%m-%d--%H-%M") + ".pkl", 'wb') as f:
